  ...
  inline void loadVisual(McRtcGui & gui, const std::string & rm_path, const rbd::parsers::Visual & visual)
  {
    ...
    switch(visual.geometry.type)
    {
      ...
      case Geometry::CYLINDER:
      {
        ...
        auto pos = visual.origin;
        const auto & start = sva::PTransformd{Eigen::Vector3d{0, 0, -cylinder.length / 2}} * pos;
        const auto & end = sva::PTransformd{Eigen::Vector3d{0, 0, cylinder.length / 2}} * pos;

        // gui_.drawArrow(translation(start), translation(end), 2 * cyl.radius, 0.0f, 0.0f, color(visual_.material));
        object = gui.makeCylinder(translation(visual.origin), convert(visual.origin.rotation()),
                                  static_cast<float>(cylinder.radius), static_cast<float>(cylinder.length),
                                  translation(start), translation(end), color(visual.material), this, group_);
        break;
      }
